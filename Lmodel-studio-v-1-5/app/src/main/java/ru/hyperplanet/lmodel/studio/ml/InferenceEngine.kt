...
object InferenceEngine {
    ...
    fun generateResponse(
        ...
        if (intent == "greeting") {
            step("5. Ответ")
            val role = systemPrompt.split(Regex("[.\\n]")).firstOrNull()?.trim()
            val ans = if (active == LanguageLock.Lang.EN)
                (if (!role.isNullOrBlank()) "Hello. $role" else "Hello!")
            else (if (!role.isNullOrBlank()) "Привет. $role" else "Привет!")
            line(ans)
            return GenerationResult(ans, log.toString().trim(), promptTokens, countTokens(ans))
        }
        if (intent == "identity") {
